...
import { usePurchaseEntryVersements } from "@/features/purchase-entries/hooks/use-purchase-entry-versements"
import type { PurchaseEntry } from "@/features/purchase-entries/types"

type PurchaseEntryVersementHistoryDialogProps = {
  entry: PurchaseEntry
}

export function PurchaseEntryVersementHistoryDialog({ entry }: PurchaseEntryVersementHistoryDialogProps) {
  const [open, setOpen] = React.useState(false)
  const { data: versements = [], isPending } = usePurchaseEntryVersements(entry.id, open)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Historique des versements — {entry.reference}</DialogTitle>
        </DialogHeader>
