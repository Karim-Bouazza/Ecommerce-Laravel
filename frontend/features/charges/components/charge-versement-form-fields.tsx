...
import type { ChargeVersementSchema } from "@/features/charges/schemas/charge-versement-schema"
import type { Charge } from "@/features/charges/types"

type ChargeVersementFormFieldsProps = {
  charge: Charge
  form: UseFormReturn<ChargeVersementSchema>
  idPrefix: string
}

export function ChargeVersementFormFields({ charge, form, idPrefix }: ChargeVersementFormFieldsProps) {
  const {
    register,
    control,
    formState: { errors },
  ...
  const isFullPayment = hasValidAmount && amountNumber >= charge.remaining_amount
  const remainderAfterPayment = charge.remaining_amount - amountNumber
      ...
      <Field data-invalid={!!errors.amount}>
        ...
        {hasValidAmount && !isFullPayment && (
          <div className="flex items-center gap-2 rounded-md border-l-4 border-amber-500 bg-amber-500/10 px-3 py-2 text-sm font-medium text-amber-600 dark:text-amber-400">
            ...
            Paiement partiel — {formatPrice(remainderAfterPayment)} restera à payer
          </div>
        )}
